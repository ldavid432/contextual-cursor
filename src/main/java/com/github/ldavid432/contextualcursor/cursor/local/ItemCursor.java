...
{
	EXPLORERS_RING_LOW(new CacheSprite(SpriteID.Magicon.LOW_LEVEL_ALCHEMY),
		hasAllOf(targetContains(" -> explorer's Ring "),
			hasAnyOf(targetEndsWith("1"), targetEndsWith("2"), targetEndsWith("3"))
		)),
	EXPLORERS_RING_HIGH(new CacheSprite(SpriteID.Magicon.HIGH_LEVEL_ALCHEMY),
		targetEndsWith(" -> explorer's ring 4")),

	PRAYER_ALTAR(new CacheSprite(SpriteID.Staticons.PRAYER),
